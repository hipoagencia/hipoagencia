function processPayment(token, buttonTarget) {
    ...
    $.ajax({
        ...
        error: function(err){
            ...
            buttonTarget.innerText = "Finalizar Compra";

            let message = (JSON.parse(err.responseText));
            document.querySelector('div.msg').innerHTML = showErrorMessages(errorsMapPagseguroAPI(message.data.message.error.code));
        }
    });
}
...
function errorsMapPagseguroAPI(code)
{
    switch(code) {
        case "53042":
            return 'Nome do dono do cartão inválido';
            break;
